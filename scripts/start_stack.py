...
class KeePassCredentials:
    ...
    def _run(self, command, *arguments, value=None):
        ...
        if result.returncode:
            raise RuntimeError("KeePassXC не выполнил операцию. Проверьте пароль, файл базы и запись ключей Vault.")
    ...
    def write(self, value):
        ...
        if self.read() != value:
            raise RuntimeError("Не удалось проверить сохранённые ключи Vault в KeePassXC.")
...
def configure_vault(client, initial):
    ...
    try:
        ...
        database = {
            ...
            "POSTGRES_PASSWORD": initial.get("POSTGRES_PASSWORD") or secrets.token_hex(32),
        }
        client.secrets.kv.v2.create_or_update_secret(path="database/postgres", secret=database)
    if initial and any(initial[key] != database[key] for key in DATABASE_KEYS):
        raise RuntimeError("Реквизиты старой БД отличаются от Vault; автоматическая замена запрещена.")
    client.secrets.kv.v2.create_or_update_secret(path="kafka/config", secret={
    ...
    for role, prefix in (("diabetes-api", "API"), ("kafka-consumer", "CONSUMER")):
        ...
        client.auth.approle.create_or_update_approle(
            role_name=role, token_policies=[role], token_ttl="1h", token_max_ttl="4h",
            secret_id_ttl="0", secret_id_num_uses=0,
        )
        # Ротация сервисных удостоверений при управляемом запуске.
        try:
            accessors = client.auth.approle.list_secret_id_accessors(role)["data"]["keys"]
        except hvac.exceptions.InvalidPath:
            accessors = []
        for accessor in accessors:
            client.auth.approle.destroy_secret_id_accessor(role, accessor)
        identities[f"{prefix}_VAULT_ROLE_ID"] = client.auth.approle.read_role_id(role)["data"]["role_id"]
        identities[f"{prefix}_VAULT_SECRET_ID"] = client.auth.approle.generate_secret_id(role)["data"]["secret_id"]
    return database, identities


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--ci", action="store_true", help="Одноразовый стенд, bootstrap-ключи только в памяти")
    parser.add_argument("--keepass-db", help="Файл KeePassXC .kdbx; мастер-пароль запрашивается скрытым вводом")
    parser.add_argument("--keepass-key-file", help="Дополнительный ключевой файл KeePassXC, если используется")
    parser.add_argument("--migrate-env", action="store_true", help="Перенести существующие реквизиты, после проверки удалить .env")
    ...
    parser.add_argument("--check", action="store_true", help="Интеграционные проверки на чистом стенде")
    ...
    store = None if args.ci else KeePassCredentials(args.keepass_db, args.project, args.keepass_key_file)
    ...
    if not client.sys.is_initialized():
        ...
        bootstrap = {"root_token": keys["root_token"], "unseal_key": keys["keys_base64"][0]}
        if store:
            store.write(bootstrap)
    else:
        if args.ci:
            raise RuntimeError("--ci требует нового тома Vault; существующее хранилище не изменено.")
    ...
    if os.getenv("GITHUB_ACTIONS") == "true":
        for value in [*bootstrap.values(), database["POSTGRES_PASSWORD"], *identities.values()]:
            print(f"::add-mask::{value}", flush=True)
    run("up", "-d", "db", "kafka")
    run("run", "--rm", "diabetes-api", "python", "-m", "src.register_release", "models/current.json", "--apply")
    run("run", "--rm", "diabetes-api", "python", "-m", "scripts.activate_model_release", "models/current.json", "--if-no-champion")
    run("up", "-d", "--no-build", "diabetes-api", "kafka-consumer")
    wait_until(lambda: requests.get(args.api_url + "/db/health", timeout=5).ok, "API не готов")
    run("exec", "-T", "diabetes-api", "python", "-m", "scripts.check_database_contract")
    if args.check:
        run("exec", "-T", "diabetes-api", "python", "-m", "scripts.check_prediction_flow")
        ...
        for path in ("secret/data/database/postgres", "secret/data/kafka/config"):
            assert service.sys.get_capabilities(paths=[path])["data"]["capabilities"] == ["read"]
        assert service.sys.get_capabilities(paths=["sys/policies/acl"])["data"]["capabilities"] == ["deny"]
        run("restart", "vault")
        wait_until(client.sys.is_sealed, "Vault не перезапустился")
        client.sys.submit_unseal_key(bootstrap["unseal_key"])
        assert client.secrets.kv.v2.read_secret_version(
            path="database/postgres", raise_on_deleted_version=True,
        )["data"]["data"] == database
        run("restart", "diabetes-api", "kafka-consumer")
        wait_until(lambda: requests.get(args.api_url + "/db/health", timeout=5).ok, "API не восстановился")
