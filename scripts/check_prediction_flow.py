...
def main():
    payload = dict(
        patient_code="ITG001", study_date="2026-09-08", pregnancies=6,
        glucose=148, blood_pressure=72, skin_thickness=35, insulin=0,
        bmi=33.6, diabetes_pedigree_function=0.627, age=50,
    )
    factory = get_session_factory()
    with factory() as db:
        existing = db.scalar(select(Study.id).where(
            Study.patient_code == payload["patient_code"],
            Study.study_date == date.fromisoformat(payload["study_date"]),
        ))
        if existing is not None:
            raise RuntimeError("Тестовое исследование уже существует; повторный запуск не проверит доставку Kafka")
        expected = set(db.scalars(select(ModelVersion.model_version).where(
    ...
    while True:
        with factory() as db:
            records = db.scalars(select(PredictionHistory).join(Study).where(
                Study.patient_code == payload["patient_code"],
                Study.study_date == date.fromisoformat(payload["study_date"]),
            )).all()
        ...
        time.sleep(1)
    assert request(payload) == first, "Повторный запрос должен вернуть сохранённый результат"
    try:
        request({**payload, "glucose": 149})
    except urllib.error.HTTPError as error:
        assert error.code == 409
    else:
        raise AssertionError("Изменение показателей существующего исследования должно вернуть 409")
    print("Проверено: API, Kafka, два прогноза, повторный запрос и конфликт показателей.")
