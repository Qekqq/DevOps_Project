...
8. При ошибке фоновой версии обработчик сохраняет задачу в `shadow_retries`.
   После сохранения результатов и задач повторов подтверждает позицию в Kafka.
   Отдельный поток consumer автоматически повторяет отложенные расчёты.
...
При повторной доставке уже сохранённые результаты пропускаются. Ошибка фоновой
модели сохраняется как задача в `shadow_retries`; другие сообщения продолжают
обрабатываться. `src/kafka/retries.py` автоматически повторяет задачи с увеличением
паузы до 30 минут, используя текущие показатели исследования. При успехе либо
выходе версии из роли challenger задача удаляется. Ошибка БД при сохранении
результата или задачи оставляет сообщение Kafka неподтверждённым.
...
- `make start` вызывает `scripts/deploy_release.py --resume`: запускает уже
  установленные контейнеры, при необходимости разблокирует Vault и проверяет
  готовность. Локальной сборки нет; затем запускается зарегистрированный runner.
- `scripts/start_stack.py` используется для первоначальной подготовки и проверок
  одноразового окружения. Сборка локальных исходников требует `--local-build`.
- `make preview` временно копирует frontend из рабочей папки в запущенный контейнер;
  следующая выкладка заменит эти файлы проверенным образом.
...
flowchart LR
    ...
    DVC --> CI
    CI --> PR["Pull request develop в main<br/>Проверки CI"]
    PR --> MAIN["Ручное слияние в main"]
    MAIN --> MCI["CI main<br/>Проверки и публикация образов"]
    MCI --> CD["CD<br/>Одноразовая проверка"]
    CD --> DEPLOY["Windows runner<br/>Копия БД, обновление, проверка готовности"]
```
...
После успешного CI для `main` CD проверяет готовые образы по digest в отдельном
окружении GitHub Actions и удаляет его. Затем self-hosted Windows runner создаёт
резервную копию PostgreSQL и обновляет приложение на локальном компьютере.
Рабочие исходники при выкладке не собираются. Vault должен быть разблокирован,
Docker и runner запущены. Push в develop не обновляет постоянное приложение;
pull request и слияние пользователь выполняет сам.

Блокировка выкладки освобождается ОС даже при аварийном завершении процесса.
При ошибке резервного копирования обновление не начинается. Восстановление БД
и возврат предыдущей версии приложения остаются ручными. Docker-журналы новых
контейнеров ограничены тремя файлами по 10 MB; существующей инфраструктуре
настройка применяется при отдельном пересоздании, с сохранением томов.
