...
пересборку. Она увеличивает время CI и требует проверки совместимости обновлений.
...
компиляторы остаются в промежуточных стадиях Docker. CI повторно использует
неизменившиеся образы по digest, чтобы не скачивать и не компилировать Go-модули
при каждом изменении приложения. Основания обновления gRPC приведены выше.

## Ограничения сканирования

Выпуск блокируют HIGH/CRITICAL с доступным исправлением, за исключением
описанных точечных оценок неприменимости. Это не означает отсутствия всех CVE:
