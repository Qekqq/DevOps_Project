...
def test_shadow_preserves_study_identity_and_skips_saved_prediction(monkeypatch, already_saved):
    db = MagicMock()
    ...
    monkeypatch.setattr(shadow, "get_prediction_for_study", lambda *args, **kwargs: {} if already_saved else None)
    predictor = Mock()
    predictor.predict.return_value = {"prediction": 1, "probability": .8}
    ...
    message = {"patient_code": "PAT001", "study_date": "2026-09-08", "model_version": "champion-v1", "features": {"glucose": 120}}
...
def test_failed_model_does_not_prevent_other_models_from_being_saved(monkeypatch):
    db = MagicMock()
    db.__enter__.return_value = db
    db.execute.return_value.scalars.return_value.all.return_value = ["broken", "working"]
    db.execute.return_value.scalar_one_or_none.side_effect = [
        SimpleNamespace(model_version="broken"), SimpleNamespace(model_version="working"),
    ]
    monkeypatch.setattr(shadow, "get_session_factory", lambda: lambda: db)
    monkeypatch.setattr(shadow, "get_prediction_for_study", lambda *args, **kwargs: None)
    def load(record):
        if record.model_version == "broken":
            raise ValueError("invalid artifact")
        return SimpleNamespace(predict=lambda features: {"prediction": 0, "probability": .2})
    monkeypatch.setattr(shadow.registry, "from_record", load)
    save = Mock()
    monkeypatch.setattr(shadow, "save_prediction_history", save)
    with pytest.raises(RuntimeError, match="broken"):
        shadow.predict_challengers({"patient_code": "PAT001", "study_date": "2026-09-08",
                                    "model_version": "champion", "features": {}})
