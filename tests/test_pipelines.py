...
def test_pipeline_uses_training_statistics_and_survives_serialization(tmp_path):
    train = pd.DataFrame(
        [
            [0, 100, 70, 20, 100, 30, 0.2, 25],
            [1, 120, 80, 40, 200, 40, 0.3, 35],
            [2, 0, 0, 0, 0, 0, 0.4, 45],
            [3, 140, 90, 30, 300, 35, 0.5, 55],
        ],
...
def test_mixed_imputation_keeps_training_statistics_order_and_serialization(tmp_path):
    train = pd.DataFrame(
        [
            [0, 100, 70, 20, 100, 30, 0.2, 25],
            [1, 110, 80, 30, 200, 35, 0.3, 35],
            [2, 210, 150, 90, 600, 80, 0.4, 45],
            [3, 0, 0, 0, np.nan, 0, 0.5, 55],
        ],
        columns=FEATURE_COLUMNS,
    )
    pipeline = build_pipeline(
    ...
    expected = [[0, 140, 80, 30, 300, 35, 0.5, 55]]
    np.testing.assert_allclose(pipeline[:-1].transform(request), expected)
    np.testing.assert_allclose(
        pipeline[:-1].transform(request[FEATURE_COLUMNS[::-1]]),
        expected,
    )
    ...
    search = GridSearchCV(
        pipeline,
        {"imputer__glucose__strategy": ["median", "mean"]},
        cv=2,
        error_score="raise",
    ).fit(train, [0, 0, 1, 1])
