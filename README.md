...
**Адрес приложения:** https://mloops.fun. Рабочая установка размещена на Ubuntu VPS.
...
```mermaid
flowchart LR
    Browser[Браузер] --> TLS[Nginx на VPS: HTTPS]
    TLS --> Nginx[Nginx frontend]
...
| HTML, CSS, JavaScript, Nginx | Интерфейс без отдельного Node-сервера; системный Nginx обслуживает HTTPS, Nginx frontend раздаёт файлы, проксирует API и проверяет доступ к Grafana |
...
| Vault, KeePassXC | Vault выдаёт секреты сервисам; KeePassXC хранит данные для ручной разблокировки и обслуживания |
...
Снимок создаётся администратором через интерфейс. Артефакты моделей хранятся в DVC;
CI получает версии, на которые ссылается проверяемый commit. Отсутствие этих
артефактов в удалённом хранилище останавливает подготовку выпуска.
...
Минимальный объём — 100 непустых значений признака в текущей и эталонной выборках.
...
изменить метрики. Сигнал drift отображается в отчёте; автоматического переобучения
по этому сигналу нет.
...
Раздел **«Работа приложения»** открывает дашборд Grafana `fastapi-observability`
за последние 24 часа. Кнопка с логотипом Grafana открывает этот же дашборд
в отдельной вкладке. Изменения дашбордов сохраняются в томе `grafana_data`.
Установлены источники Prometheus и Loki;
...
Сессии проверяются по БД и действуют восемь часов. На VPS cookie имеет Secure,
HttpOnly и SameSite=Strict; изменяющие запросы защищены CSRF. Отключение пользователя
или смена его пароля отзывает доступ при следующем запросе.
Nginx и API ограничивают частоту и размер запросов.
...
только для чтения и не включаются в новые образы приложения. На VPS настроены
Vault AppRole, передача идентификаторов через файлы и TLS с проверкой сертификата.

Команды резервного копирования локального стенда:
...
подтверждает формат архива, но не заменяет проверку восстановления. Копии с VPS
сохраняются на компьютер вручную; отправка во внешнее хранилище по расписанию
не настроена. Расшифровка создаёт новый файл
...
Оценки неприменимости CVE описаны в `scripts/vulnerability_review.py`. Каждая запись
содержит CVE, пакет, версию, путь, SHA-256 бинарного файла, даты проверки и пересмотра,
ссылку на доказательства. Тесты проверяют возврат к обычной блокировке при изменении
условий оценки. CI применяет оценки из проверяемого commit автоматически: находка остаётся
...
`requirements-security.txt` содержит зависимости отчётов и их тестов; библиотека PDF
...
Служебные образы обслуживает отдельный workflow `Infrastructure` с группами
`monitoring` и `infrastructure`. Ручной запуск доступен через
**Actions → Infrastructure → Run workflow** после размещения workflow в основной ветке.
Также реализован запуск по Git-меткам `infra-monitoring-*` и `infra-infrastructure-*`.
Задание получает готовые образы или собирает изменившиеся, проверяет и публикует их;
приложение при этом не выкладывается. Опубликованные версии использует обычный CI.
...
CD скачивает готовые образы на временный интеграционный стенд и проверяет
совместную работу сервисов. На этом этапе образы не пересобираются.

В GitHub созданы секреты `DVC_YANDEX_USER`, `DVC_YANDEX_PASSWORD`,
...
Actions → Variables** сохранены `VPS_HOST` (адрес сервера) и `VPS_USER` (пользователь
выкладки). Ключ добавлен в `authorized_keys` этого пользователя с опцией `restrict`.
Она отключает туннели и интерактивный терминал, но разрешает команды выкладки.
Пользователь выкладки включён в группу `docker` и выполняет команды Docker без
запроса пароля. Членство в группе `docker`
даёт полномочия уровня root; это техническая учётная запись управления сервером.

На VPS установлены Docker Engine, Docker Compose и системный Python 3.12;
приложение выполняется в контейнере с Python 3.11.
...
Первоначальная установка выполнена отдельно от CD: на VPS перенесены данные,
настроены Vault и внешний Nginx, проверены HTTPS и соединение приложения с БД.
После успешного запуска создан `deployment/current.json` с данными выпуска.
CD проверяет эту запись и обновляет существующую установку; перенос с Windows
и создание пустой базы в обычную выкладку не входят. Автоматического отката БД нет.
...
`DOCKERHUB_TOKEN` используется в CI и `Infrastructure` с правами
**Read & Write** для скачивания и публикации образов Docker Hub.
...
Для замены установленных образов PostgreSQL, Kafka и Vault реализован отдельный
процесс миграции с резервным копированием и журналом восстановления.
При несовпадении этих образов обычный CD останавливается на предварительной проверке.
Команды миграции локального стенда используют проверенный пакет выпуска:
...
командой `make security-restore` с доступом к KeePassXC. Подтверждённая миграция
снимает блокировку последующих выкладок.
...
Приложение доступно по **https://mloops.fun**. На сервер перенесены пользователи,
исследования, прогнозы, обратная связь, файлы снимков и сохранённые дашборды Grafana.
История технических метрик и журналов на VPS начинается с запуска серверной установки.

Настроены системный Nginx, HTTPS и таймер продления сертификата Certbot, Docker
и 13 контейнеров проекта. Внешний Nginx передаёт запросы в frontend на
`127.0.0.1:8080`; Vault доступен с хоста на `127.0.0.1:8201` по TLS.
PostgreSQL, Kafka и мониторинг используют внутренние Docker-сети.
Публичный доступ к Grafana проходит через авторизацию администратора приложения.

| Где хранятся файлы | Назначение |
|---|---|
| Docker-тома VPS | PostgreSQL, Vault, Kafka, Grafana и накопленные метрики/журналы |
| `/opt/devops_project/deployment/current.json` | Commit, CI run и каталог установленного выпуска; обновляется скриптом выкладки |
| `/opt/devops_project/deployment/releases/` | Проверенные конфигурации, модели и настройки конкретных выпусков |
| `/opt/devops_project/deployment/data/feedback/` | Файлы снимков данных с обратной связью |
| `/opt/devops_project/deployment/backups/` | Копии БД; JSON дашбордов, сохранённый при переносе |
| `/opt/devops_project/admin-tools/` | Подготовленные команды создания пользователей и резервного копирования |
| Компьютер владельца | Исходники, KeePassXC, SSH-ключи и локальные копии восстановления; не нужны для обычной работы сайта |
...
```powershell
ssh -o IdentitiesOnly=yes -i "$env:USERPROFILE\.ssh\mloops_selectel_ed25519_v2" deploy@135.106.211.247
```

Пароль ключа вводится в терминале. После подключения команды Linux выполняются
в приглашении `deploy@mloops:~$`. Команда `exit` закрывает SSH, сайт продолжает работать.
Личный SSH-ключ используется для ручного управления; отдельный ключ `mloops_cd_ed25519`
предназначен для автоматизации. Пользовательские аккаунты приложения и техническая
учётная запись SSH разделены.

### Пользователи и возможности администратора

| Роль | Что доступно |
|---|---|
| `user` | Создание прогнозов |
| `admin` | Прогнозы, история, исправления исследований, фактические исходы, снимки данных и оба раздела мониторинга |
| Владелец с SSH/Docker-доступом | Создание и отключение пользователей, обслуживание и выкладка |

Создание пользователей выполняется серверной командой, вне веб-интерфейса.
Пример создания нового аккаунта `new_user` с ролью `admin` — **на VPS после подключения по SSH**:

```bash
cd /opt/devops_project/admin-tools
python3 -m scripts.create_user new_user --role admin
```

Команда получает пароль скрытым вводом и сохраняет его хеш в БД.
Существующая учётная запись с тем же логином не перезаписывается.

### Проверка состояния и перезапуск

Основные команды **на VPS**:

```bash
docker ps -a --filter label=com.docker.compose.project=devops_project
curl -fsS https://mloops.fun/api/health
curl -fsS https://mloops.fun/api/db/health
...
`health` проверяет API, `db/health` — соединение с PostgreSQL. У контейнеров с
настроенным healthcheck ожидается `healthy`, у остальных — `Up`.
Технические графики доступны в **Мониторинг → Работа приложения**, качество моделей —
в **Мониторинг → Метрики моделей**. По умолчанию технический дашборд открывается за 24 часа.

Последние сообщения API и фонового обработчика:

```bash
docker logs --since 10m --tail 100 devops_project-diabetes-api-1
docker logs --since 10m --tail 100 devops_project-kafka-consumer-1
```

Перезапуск процессов приложения без пересборки и удаления данных:

```bash
docker restart devops_project-diabetes-api-1 devops_project-kafka-consumer-1 devops_project-metrics-exporter-1
```

Обычная выкладка пересоздаёт контейнеры обновляемых сервисов и сохраняет постоянные
тома. `scripts.bootstrap_vps --apply` предназначен только для первой установки
и отклоняет повторный запуск на уже подготовленном сервере.

### После перезагрузки VPS

Docker и Nginx включены в автозапуск. Контейнеры используют `unless-stopped`:
после перезагрузки они запускаются, если ранее не были остановлены вручную.
Vault после своего перезапуска остаётся запечатанным.

Команда разблокировки Vault выполняется в **PowerShell из каталога проекта**,
с установленным KeePassXC CLI и доступной базой KeePass:

```powershell
.\.venv\Scripts\python.exe -m scripts.bootstrap_vps --unseal --recovery-file ".local-history/vps-transfer/cutover-20260915T155743Z/vault-recovery.json"
```

Скрипт запрашивает мастер-пароль KeePass, расшифровывает ключ локально и передаёт его
на VPS по SSH. Готовность приложения после разблокировки определяется по `health`
и `db/health`; для перезапуска API, consumer и exporter используется команда
`docker restart` из предыдущего раздела. Автоматической разблокировки Vault внешним KMS нет.

### Резервные копии и обслуживание

CD создаёт копию PostgreSQL перед обновлением. Дополнительное резервирование
выполняется вручную. Ежедневного расписания и автоматической отправки во внешнее
хранилище нет. Команда создания копии **на VPS**:

```bash
cd /opt/devops_project/admin-tools
python3 -m scripts.backup_database --directory /opt/devops_project/deployment/backups --container devops_project-db-1
```

Команда проверяет формат архива и его оглавление; это не замена проверке полного
восстановления. Ротация после успешного CD сохраняет две последние копии и защищённые
архивы восстановления. Между выкладками ручные копии могут накапливаться.

Копирование архивов и файлов снимков на компьютер — **PowerShell из каталога проекта**:

```powershell
New-Item -ItemType Directory -Force .local-history/vps-backup | Out-Null
scp -r -i "$env:USERPROFILE\.ssh\mloops_selectel_ed25519_v2" deploy@135.106.211.247:/opt/devops_project/deployment/backups .local-history/vps-backup/
scp -r -i "$env:USERPROFILE\.ssh\mloops_selectel_ed25519_v2" deploy@135.106.211.247:/opt/devops_project/deployment/data/feedback .local-history/vps-backup/
```

Копия БД включает пользователей и исследования. Docker-тома Vault, Grafana,
Prometheus и Loki в этот архив не входят. Файл `grafana-dashboards-cutover.json`
содержит настройки дашбордов на момент переноса; последующие изменения сохраняются
в томе Grafana и экспортируются в JSON отдельно.
Зашифрованный `vault-recovery.json` содержит ключи разблокировки Vault, но не его
хранилище секретов. Таким образом, текущий набор архивов не является полной
резервной копией всех постоянных данных VPS.

База KeePass, SSH-ключи и каталог первоначального переноса
`.local-history/vps-transfer/cutover-20260915T155743Z/` хранятся на компьютере владельца.
Локальные архивы и файлы восстановления исключены из Git через `.gitignore`.
Проверки восстановления выполняются в отдельной среде, без замены рабочей БД.

Проверка продления HTTPS-сертификата **на VPS**:
...
Certbot продлевает сертификат по таймеру; `--dry-run` выполняет проверку продления.
Регистрация домена и аренда VPS обслуживаются отдельно от сертификата.
Описание механизма — [документация Certbot](https://eff-certbot.readthedocs.io/en/stable/using.html#renewing-certificates).
...
Синтаксис JavaScript проверяется в Node.js 22: `node --check frontend/app.js`
и аналогично для файлов `frontend/js/*.js`. Интеграционные тесты запускаются
на одноразовом стенде через `scripts.start_stack --ci --check` с отдельным именем
проекта, портами и томами. Конфигурация проверки выпуска описана в `.github/workflows/cd.yml`.

`make preview` публикует локальные файлы интерфейса на `http://localhost:8081`.
Preview использует API локального стенда и доступен при запущенном локальном frontend.
Совместимость новых маршрутов зависит от установленной локальной версии backend.
...
| `infrastructure/` | Инфраструктурные образы, конфигурация внешнего Nginx и доказательства применимости CVE |
...
и локальные секреты исключены из Git. `.local-history/` содержит состояние локального
стенда, выпуски и данные восстановления, наряду с временными файлами.
...
Собственная сборка сервера входит в ручное обслуживание образов; совместимость
с интерфейсом и плагинами проверяется на интеграционном стенде.
