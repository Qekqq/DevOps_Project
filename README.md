...
не входят в рабочие контейнеры. Обычный CI использует уже опубликованные образы
...
`DOCKERHUB_TOKEN` относится к Docker Hub, а не к GitHub: для CI и `Infrastructure`
нужны **Read & Write** (скачивание и публикация образов), **Delete** не требуется.
Сейчас CD и плановая проверка используют тот же секрет, хотя им достаточно **Read**;
при настройке сервера стоит выделить отдельный токен только для чтения.
Токены старых неиспользуемых интеграций следует отозвать после проверки их назначения.
...
не в файлах проекта или отчётах.
