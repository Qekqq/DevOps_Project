...
def read_raw_dataset(path: Path):
    ...
    if len(frame.columns) != len(columns) or set(frame.columns) != set(columns):
        raise ValueError("Ожидаются восемь признаков и outcome без лишних или повторяющихся колонок")
    if frame.empty:
        raise ValueError("Исходный датасет пуст")
    frame = frame.loc[:, columns].apply(pd.to_numeric, errors="raise")
    if not np.isfinite(frame.to_numpy()).all():
        raise ValueError("Исходный датасет содержит пустые или бесконечные значения")
    ...
    return frame, {"sha256": sha256(content).hexdigest(), "rows": len(frame),
