...
from src.db.repositories import DuplicatePredictionError, get_prediction_for_study, save_prediction_history
...
def predict_challengers(message):
    ...
    with session_factory() as db:
        versions = list(db.execute(select(ModelVersion.model_version).where(
    ...
    for version in versions:
        try:
            with session_factory() as db:
                ...
                model = db.execute(select(ModelVersion).where(
                    ModelVersion.model_version == version,
                    ModelVersion.role == "challenger",
                )).scalar_one_or_none()
                if model is None:
                    continue
                saved = get_prediction_for_study(db, patient_code=message["patient_code"],
                    study_date=study_date, model_version=version, features=message["features"])
                if saved is not None:
                    continue
                started = time.perf_counter()
                result = registry.from_record(model).predict(message["features"])
                save_prediction_history(db, features=message["features"], prediction=result["prediction"],
                    probability=result.get("probability"), model_version=model, study_date=study_date,
                    patient_code=message["patient_code"], request_source=message.get("request_source", "api"),
